...
NOTE: If JBoss Data Virtualization Server is already started from the command line, JBoss Developer Studio will show a warning "Server already running on localhost", select option "Set the server adapter to 'started', without launching a new server."   
